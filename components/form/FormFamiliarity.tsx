 ...
 */
export const FormFamiliarity = ({ label, name, control, ...props }: FormFamiliarityProps) => (
  <Controller
    name={name}
    control={control}
      ...
      <>
        <FormControl>
          ...
          <Rating
            ...
            icon={<Image src="/images/Rated.svg" alt="Rated" width={15} height={25} />}
            emptyIcon={<Image src="/images/UnRated.svg" alt="Rated" width={15} height={25} />}
            sx={{ '& .MuiRating-icon': { marginRight: '15px' } }}
          />
          {value ? <Box sx={{ mt: 1, color: '#C6C6C6' }}>{labels[value as keyof typeof labels]}</Box> : <></>}
        </FormControl>
      </>
    )}
  />
